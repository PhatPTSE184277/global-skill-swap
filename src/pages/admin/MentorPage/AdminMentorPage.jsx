...
const AdminMentorPage = () => {
  ...
  useEffect(() => {
    ...
    fetchMentors(params);
  }, [fetchMentors, page, size, applicationStatus]);
  ...
  const pendingMentors = filteredMentors.filter(
    (m) => !m.isApproved && !m.isRejected
  );
  const allPendingSelected =
    pendingMentors.length > 0 &&
    pendingMentors.every((m) => selectedIds.includes(m._id || m.id));
    ...
    <div className="max-w-7xl mx-auto mt-8">
      ...
      {selectedIds.length > 0 && (
        <div className="mb-4 flex gap-3">
          ...
          >
            <FaCheckCircle className="text-white drop-shadow" /> Duyệt (
            {selectedIds.length})
          </button>
          <button
          ...
          >
            <FaTimesCircle className="text-white drop-shadow" /> Từ chối (
            {selectedIds.length})
          </button>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div className="w-48">
          <AdminSelect
            ...
            placeholder="Lọc theo trạng thái CV"
          />
        </div>
