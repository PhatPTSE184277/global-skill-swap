...
const PostCreate = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await createPost({ title, content, isPublic });
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              className="ml-auto text-[#4D2C5E] text-xs underline hover:text-[#3c204a] transition cursor-pointer"
          ...
          {showPreview && (
            <div className="border border-purple-200 rounded-xl p-4 bg-purple-50 text-gray-800 text-sm max-h-60 overflow-auto">
          ...
          >
            {loading ? <Loader2 className="animate-spin mr-2" size={20} /> : "Đăng bài viết"}
          </button>
        </form>
      </motion.div>
    </div>
