...
const ProductEdit = ({ isOpen, onClose, product }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Cập nhật gói mentor thành công!');
      onClose();
    } catch (error) {
      toast.error('Có lỗi xảy ra khi cập nhật gói mentor');
      console.error(error);
    } finally {
      setLoading(false);
    }
    ...
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl p-6 w-[90vw] max-w-md" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-semibold text-gray-800">Chỉnh sửa gói mentor</h3>
          <button
